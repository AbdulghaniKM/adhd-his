      ...
      <template #cell-name="{ row }">
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <div class="text-xs text-text-secondary">{{ row.department?.title || 'No Department' }}</div>
          </div>
        </div>
      </template>
...
const formFields = computed<FormFieldRow[]>(() => [
  ...
  [
    { key: 'Password', label: 'Account Password', type: 'password', placeholder: 'Keep empty to leave unchanged if editing' },
  ],
]);
